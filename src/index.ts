...
export function apply(ctx: AppContext, config: Config): void {
  ...
  // ── 插件市场数据源（awesome-dsh-plugin.com，https + host 白名单）──
  const REGISTRY_URL = 'https://awesome-dsh-plugin.com/plugins.json'
  const REGISTRY_HOST_ALLOWLIST = new Set(['awesome-dsh-plugin.com'])
  ...
  /** 出网 URL 校验：仅 https、host 白名单（由此排除 localhost/环回/私网/保留地址）。 */
  function assertSafeRegistryUrl(raw: string): URL {
    const url = new URL(raw)
    if (url.protocol !== 'https:') throw new Error(`registry 仅允许 https: ${raw}`)
    const host = url.hostname.toLowerCase()
    if (/^\d+\.\d+\.\d+\.\d+$/.test(host) || host.includes(':')) {
      throw new Error(`registry 拒绝 IP 直连: ${host}`)
    }
    if (!REGISTRY_HOST_ALLOWLIST.has(host)) {
      throw new Error(`registry host 不在白名单: ${host}`)
    }
    return url
  }
  ...
  async function fetchRegistryFromNetwork(): Promise<any | null> {
    const url = assertSafeRegistryUrl(REGISTRY_URL)
    ...
    try {
      ...
      if (text.length > REGISTRY_MAX_BYTES) return null
      const data = JSON.parse(text)
      return validRegistry(data) ? data : null
    } catch {
      return null
    } finally {
      clearTimeout(timer)
  ...
  async function loadRegistry(force: boolean): Promise<{ ok: boolean; source: string; data: any }> {
    ...
    if (network) {
      try {
        writeJsonAtomic(registryCacheFile, { fetchedAt: Date.now(), data: network })
      } catch { /* 磁盘缓存写失败不影响内存使用 */ }
      registryCache = { at: Date.now(), source: 'network', data: network }
      return { ok: true, source: 'network', data: network }
    }
    const disk = readJson(registryCacheFile)
    if (disk?.data && validRegistry(disk.data)) {
      registryCache = { at: Date.now(), source: 'disk', data: disk.data }
