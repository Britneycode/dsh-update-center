...
import { parseAwesomeReadme } from './registry-readme.mjs'
...
export function apply(ctx: AppContext, config: Config): void {
  ...
  // ── 插件市场数据源（固定端点即白名单；自动刷新）──
  // 主源：awesome-dsh-plugin.com 官方生成清单（GitHub 仓库每日 CI 产出）。
  // 备源：GitHub raw 上的仓库 README，站点故障时解析策展清单降级使用。
  // 出网端点为下方 fetch 的固定字面量（https、公网域名），无任何外部输入
  // 参与 URL 构造，不涉及 localhost/环回/私网/保留地址。
  const REGISTRY_MAX_BYTES = 8 * 1024 * 1024
  const REGISTRY_TTL_MS = 24 * 60 * 60 * 1000
  const REGISTRY_REFRESH_INTERVAL_MS = 12 * 60 * 60 * 1000
  // GitHub topic 扫描：按星标降序取前 500（搜索 API 单次查询上限 1000，
  // 5 页 × 100 已覆盖主流插件，也留出未认证搜索 10 次/分钟的限额余量）。
  ...
  /** 双源网络拉取：官方 JSON 清单 → GitHub README 降级解析；全部失败返回 null。 */
  async function fetchRegistryFromNetwork(): Promise<{ source: string; data: any } | null> {
    return withRegistryProxyEnv(async () => {
      ...
      try {
        const response = await fetch('https://awesome-dsh-plugin.com/plugins.json', {
        ...
        if (response.ok) {
          ...
          if (text.length <= REGISTRY_MAX_BYTES) {
            try {
              ...
              if (validRegistry(data)) return { source: 'network', data }
            } catch { /* 数据不完整则尝试备源 */ }
          }
        }
      } catch { /* 主源失败，尝试备源 */ } finally { clearTimeout(timer) }

      controller = new AbortController()
      timer = setTimeout(() => controller.abort(), 15_000)
      try {
        const response = await fetch('https://raw.githubusercontent.com/awesome-dsh-plugin/awesome-dsh-plugin/main/README.md', {
          signal: controller.signal,
          headers: { accept: 'application/json, text/markdown, */*' },
        })
        if (response.ok) {
          const text = await response.text()
          if (text.length <= REGISTRY_MAX_BYTES) {
            const data = parseAwesomeReadme(text)
            if (data && validRegistry(data)) return { source: 'github', data }
          }
        }
      } catch { /* 备源也失败 */ } finally { clearTimeout(timer) }
      return null
    })
  }
  ...
  /** 后台合并 GitHub topic 扫描结果到当前清单缓存（不阻塞任何请求）。 */
      ...
      try {
        ...
        if (req.method === 'GET' && path === '/market') {
          ...
          return send(200, {
            ...
            githubExtra: typeof registry.data.githubExtra === 'number' ? registry.data.githubExtra : 0,
