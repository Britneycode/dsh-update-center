...
export function apply(ctx: AppContext, config: Config): void {
  ...
  let startJobChain: Promise<unknown> = Promise.resolve()
  function startJob(spec: Record<string, unknown>): Promise<Record<string, unknown>> {
    const run = startJobChain.then(() => startJobInner(spec))
    startJobChain = run.then(() => undefined, () => undefined)
    return run
  }

  async function startJobInner(spec: Record<string, unknown>): Promise<Record<string, unknown>> {
  ...
  async function enrichRegistryWithGithubTopics(): Promise<void> {
    ...
    const now = Date.now()
    if (now - lastGithubTopicScanAt < GITHUB_TOPIC_MIN_INTERVAL_MS) return
        ...
        if (path === '/restart') {
          ...
          try {
            ...
            for (const arg of [restartTarget, restartSpecPath]) {
              if (!isAbsolute(arg) || arg.startsWith('-')) throw new Error(`restart 路径非法: ${arg}`)
            }
