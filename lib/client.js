		...
		function buildPanel() {
			...
			function pluginUpdateCount(plugins) {
				return plugins.filter((plugin) => {
					...
					return plugin.kind === "npm" && plugin.latest && plugin.latest !== plugin.version || (plugin.kind === "link" || plugin.kind === "preset") && Number(git.behind) > 0 && !git.dirty;
				}).length;
			}
			...
			*  插件的更新按钮仍然可用，不必重新“检查更新”。 */
			...
			function bindAction(button, path, body, confirmation, startingNote) {
			...
			function renderMarket() {
				...
				for (const entry of filtered.slice(0, marketVisibleLimit)) {
					...
					const updatable = statusPlugin && (statusPlugin.kind === "npm" && statusPlugin.latest && statusPlugin.latest !== statusPlugin.version || Number(statusPlugin.git?.behind) > 0);
					const actions = el("div", "uc-actions");
			...
			async function loadMarket(force, silent) {
				...
				try {
					const data = await fetchJson(force ? "/market/refresh" : "/market", force ? { method: "POST" } : void 0);
