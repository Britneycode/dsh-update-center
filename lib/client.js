window.__ModuleLoader__.load({
	id: "@dsh-external/dsh-update-center",
	factory: (require) => {
		var module = { exports: {} };
...
`;
		...
		function buildPanel() {
			...
			function renderMarket() {
				...
				marketMeta.textContent = `来源：${{
					registry: "本仓库 plugins.json",
					github: "GitHub topic:dsh-plugin",
					disk: "本地缓存",
					snapshot: "内置快照",
					network: "网络"
				}[String(marketData.source)] ?? marketData.source} · 扫描于 ${String(marketData.updated || "未知")} · 共 ${plugins.length} 个，筛选出 ${filtered.length} 个${filtered.length > 300 ? "（仅显示前 300）" : ""}`;
				if (!filtered.length) {
		...
		function apply(ctx) {
			ctx.effect(() => ctx.slots.inject("settings.section", () => ctx.slots.register({
				...
				order: 60,
				label: () => "更新中心"
			}, UpdateCenterPanel)), "@dsh-external/dsh-update-center: panel");
		}
		//#endregion
		exports.apply = apply;
