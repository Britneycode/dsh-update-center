		...
		function buildPanel() {
			...
			function renderMarket() {
				...
				marketMeta.textContent = `来源：${{
					network: "awesome-dsh-plugin.com",
					github: "GitHub 清单（降级）",
					disk: "本地缓存",
					snapshot: "内置快照"
				}[String(marketData.source)] ?? marketData.source} · 清单更新 ${String(marketData.updated || "未知")} · 共 ${plugins.length} 个，筛选出 ${filtered.length} 个${filtered.length > 300 ? "（仅显示前 300）" : ""}`;
				if (!filtered.length) {
